...
const getMentorAnnouncements = async (req, res) => {
  try {
    ...
    const announcements = await Announcement.find({ batch: { $in: batchIds } })
      .populate("batch", "name track")
      .populate("author", "name email")
      .sort({ publishDate: -1 });
...
const getAnnouncementById = async (req, res) => {
  try {
    ...
    if (!mentorBatch) {
      return res.status(403).json({
        success: false,
        message: "You cannot access this announcement",
      });
    }
...
const updateAnnouncement = async (req, res) => {
  try {
    ...
    const { title, content, targetAudience, publishDate } = req.body;
