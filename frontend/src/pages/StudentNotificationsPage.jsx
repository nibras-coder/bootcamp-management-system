...
const getNotificationMessage = (notification) => {
  ...
  if (type === "ASSIGNMENT_CREATED") {
    return metadata?.assignmentTitle
      ? `${metadata.createdBy || "Admin"} created a new assignment: ${metadata.assignmentTitle}`
      : message;
  }
  ...
  if (type === "RESOURCE_ADDED") {
    return metadata?.resourceTitle
      ? `${metadata.uploadedBy || "Admin"} uploaded a new resource: ${metadata.resourceTitle}`
      : message;
  }
  ...
  if (type === "NEW_PHASE") {
    return metadata?.phaseName
      ? `New phase "${metadata.phaseName}" started in ${metadata.batchName}`
      : message;
  }
...
const StudentNotificationsPage = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      {/* Header */}
      <div className="student-page-heading bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-6">
