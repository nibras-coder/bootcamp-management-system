...
import { Search, ChevronRight, CheckCircle, Clock, AlertTriangle, FileText, Check } from "lucide-react";
...
const ApplyPage = () => {
  ...
  const handleSubmitPhase = async (e) => {
    ...
    setSubmitting(true);
    try {
      const response = await API.post(`/applications/${application._id}/submit`, {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-8 text-center">
          ...
          <p className="text-gray-600 mb-8">You have successfully passed all admission phases and are now fully enrolled in {application.batch?.name || "your batch"}.</p>
          <button onClick={() => navigate("/student-dashboard")} className="w-full bg-teal-600 text-white font-bold py-3 px-4 rounded-xl hover:bg-teal-700 transition-colors">
            Go to Dashboard
          </button>
        </div>
      </div>
  ...
  if (application) {
    ...
    const batchData = batches.find(b => b._id === application.batch?._id || b._id === application.batch) || application.batch;
    
    if (!batchData || !batchData.phases) {
      return <div>Error loading batch phase configuration.</div>;
    }

    const phases = [...batchData.phases].sort((a, b) => a.order - b.order).filter(p => p.isActive);
    const currentPhaseConfig = phases.find(p => p.order === currentPhaseOrder);
    const existingSubmission = application.submissions?.find(s => s.phaseId === currentPhaseConfig?._id);
      ...
      <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 mb-8">
            <h3 className="text-lg font-bold text-gray-900 mb-6">Admission Phases</h3>
            <div className="space-y-4">
              {phases.map((phase) => {
                ...
                return (
                  <div key={phase._id} className={`flex items-start p-4 rounded-xl border ${isCurrent ? 'border-teal-500 bg-teal-50/30' : 'border-gray-100 bg-gray-50/50'}`}>
                    ...
                    <div className="ml-4 flex-1">
                      <h4 className={`font-semibold ${isPast ? 'text-green-700' : isCurrent ? 'text-teal-900' : 'text-gray-500'}`}>
                        {phase.name}
                      </h4>
                      {isCurrent && phase.shortMessage && (
                        <p className="text-sm text-gray-600 mt-1">{phase.shortMessage}</p>
                      )}
                    </div>
                    {isCurrent && isPendingReview && (
                       <span className="text-xs font-bold text-yellow-600 bg-yellow-100 px-2 py-1 rounded">PENDING REVIEW</span>
                    )}
                  </div>
                );
              })}
          ...
          {!isRejected && currentPhaseConfig && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="p-6 border-b border-gray-100 bg-teal-50/30">
                <h3 className="text-xl font-bold text-gray-900">Current Phase: {currentPhaseConfig.name}</h3>
                {currentPhaseConfig.longMessage && (
                  <p className="text-gray-600 mt-2 text-sm whitespace-pre-wrap">{currentPhaseConfig.longMessage}</p>
                )}
              </div>
              ...
              <div className="p-6">
                {isPendingReview ? (
                  <div className="text-center py-8">
                    ...
                    <h3 className="text-lg font-bold text-gray-900 mb-2">Under Review</h3>
                    <p className="text-gray-600">You have successfully submitted your information for this phase. The admission team is currently reviewing it.</p>
                  </div>
                ) : (
                  <form onSubmit={handleSubmitPhase} className="space-y-6">
                    {currentPhaseConfig.fields?.length === 0 ? (
                      <div className="bg-blue-50 text-blue-800 p-4 rounded-xl text-sm flex items-start">
                         ...
                         <p>No specific details are required for this phase. Simply acknowledge and submit to proceed.</p>
                      </div>
                    ) : (
          ...
          {isRejected && (
            <div className="bg-red-50 border border-red-200 rounded-2xl p-6 mt-6">
              ...
              <p className="text-red-700">We regret to inform you that your application has been rejected at this stage. Thank you for your interest.</p>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold text-gray-900 mb-8 text-center">Open Batches</h1>
        <div className="grid gap-6 md:grid-cols-2">
          {batches.length === 0 ? (
            <div className="col-span-2 text-center text-gray-500 py-12">No active batches open for application right now. Check back later!</div>
          ) : (
            batches.map((batch) => (
              <div key={batch._id} className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
                <h3 className="text-xl font-bold text-gray-900 mb-2">{batch.name}</h3>
                {batch.track && <p className="text-teal-600 font-medium text-sm mb-4">Track: {batch.track}</p>}
