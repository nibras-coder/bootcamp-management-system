...
function Announcements() {
  ...
  const filteredAnnouncements = useMemo(() => {
    ...
    return announcements.filter((announcement) => {
      ...
      const announcementDate = getDateKey(
      ...
      if (dateFilter === "custom") {
        ...
        return announcementDate === customDate;
      }
  ...
  const totalViews = announcements.reduce(
    (sum, announcement) =>
      sum +
      Number(
        announcement.viewedStudents || 0
      ),
    0
  );

  const totalStudents = announcements.reduce(
    (max, announcement) =>
      Math.max(
        max,
        Number(
          announcement.totalStudents || 0
        )
      ),
    0
  );
  ...
  const handleSubmit = async (e) => {
    ...
    if (!editingAnnouncement && !formData.batch) {
      alert("Please enter your batch ID.");
      return;
    }
              ...
              <div className="flex items-center justify-between p-6 border-b border-gray-100">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Share an important update with
                    your students.
                  </p>
                </div>
                ...
                {!editingAnnouncement && (
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-2">
                      Your announcement will only be
                      published to the batch assigned
                      to you.
                    </p>
                  </div>
                )}
