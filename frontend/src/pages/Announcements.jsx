...
function Announcements() {
  ...
  const [formData, setFormData] = useState({ title: "", content: "", targetAudience: "batch" });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await api.post("/announcements", { ...formData, batch: myBatchId });
          ...
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
            <div className="bg-white rounded-xl p-6 w-full max-w-md">
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                <button type="submit" className="w-full bg-teal-800 text-white py-2.5 rounded-lg text-sm hover:bg-teal-900">Publish Announcement</button>
              </form>
            </div>
          </div>
        )}
